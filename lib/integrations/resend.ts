...
export function getOrderConfirmationTemplate(order: EmailOrder) {
  ...
  return `<div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;color:#1e293b"><h2 style="color:#0f766e">COD order received</h2><p>Hello ${escapeHtml(order.customerName)},</p><p>We received your cash-on-delivery order <strong>${escapeHtml(order.publicId)}</strong>. We will confirm the order and prescription requirements (where applicable) before dispatch.</p><table style="width:100%;border-collapse:collapse"><thead><tr><th style="text-align:left;padding:8px">Item</th><th style="padding:8px">Qty</th><th style="text-align:right;padding:8px">Price</th></tr></thead><tbody>${items}</tbody></table><p style="text-align:right"><strong>Total due on delivery: ${money(order.grandTotalPaise)}</strong></p><p>Track your order: <a href="${siteUrl}/frames/orders/${encodeURIComponent(order.publicId)}">${escapeHtml(order.publicId)}</a></p></div>`;
}
