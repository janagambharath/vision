...
interface OrderDetail {
  ...
  payments: Array<{ providerOrderId: string | null }>;
}

export default function RazorpayPayPage({ params }: { params: Promise<{ publicOrderId: string }> }) {
  ...
  useEffect(() => {
    ...
    async function fetchOrder() {
      try {
        const response = await fetch(`/api/orders?id=${publicOrderId}`);
        if (!response.ok) {
  ...
  const startPayment = async () => {
    ...
    try {
      ...
      const options = {
        ...
        theme: {
          color: "#0f766e" // Vision Vistara Teal
        },
        modal: {
          ondismiss: function () {
            setLoading(false);
          ...
          <div>
            ...
            {order ? (
              <div className="mt-4 rounded-vv bg-slate-50 p-4 border border-slate-100">
                <p className="text-sm text-slate-500">Order ID: <strong>{order.publicId}</strong></p>
