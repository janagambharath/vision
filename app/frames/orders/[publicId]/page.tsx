      ...
      <div className="vv-container">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_380px]">
          ...
          <div className="grid gap-6 self-start">
            <aside className="vv-card p-6 bg-white border border-slate-100"><ClipboardList className="h-6 w-6 text-retail" /><h2 className="mt-3 text-xl font-extrabold">Summary</h2><dl className="mt-5 grid gap-3 text-sm"><Row label="Delivery" value={order.deliveryMethod.replace(/_/g, " ")} /><Row label="Payment" value={order.paymentMethod === "COD" ? "Cash on delivery" : latestPayment?.status === "PAID" ? "Paid" : "Awaiting confirmation"} /><div className="border-t border-slate-100 pt-3" /><Row label="Subtotal" value={formatMoney(order.subtotalPaise)} /><Row label="Lens add-ons" value={formatMoney(order.lensTotalPaise)} /><Row label="Delivery" value={formatMoney(order.shippingPaise)} />{order.taxPaise > 0 ? <Row label="Legacy tax" value={formatMoney(order.taxPaise)} /> : null}<Row label="Grand total" value={formatMoney(order.grandTotalPaise)} strong /></dl></aside>
